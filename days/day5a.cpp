...
int main() {
    ...
    while (getline(file, line)) {
        if (line.find("map") != string::npos) {
            ...
            for (int i = 0; i < seed_nums.size(); i++) {
                updated[i] = false;
            }
        } else {
            ...
            map >> range_value;
            ranges.push_back(range_value);
            long long int origin_start = ranges.at(1);
            long long int origin_end = ranges.at(1) + ranges.at(2) - 1;
            long long int destination_start = ranges.at(0);
            long long int destination_end = ranges.at(0) + ranges.at(2) -1;
            for (long int i = 0; i < seed_nums.size(); i++) {
